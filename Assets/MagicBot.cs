...
public class MagicBot : MonoBehaviour
{
    ...
    private void CompareResults()
    {
        ...
        {
            finalResultPlayer++;
            Debug.Log($"Wygrywa gracz i dostaje punkt a punkt bota to {enemyScore}");
        ...
        else if (ShowPLayerScore() < ShowEnemyScore())
        {
            ...
            enemyText.text = $"Wynik Arcybota: {finalResultEnemy}";
        }
        else
        {
            Debug.Log("Remis");
    ...
    public IEnumerator ChooseDice()
    {
        ...
        while (i < 4)
        {
            ...
            if (i==3 && randomNumbers == 0)
            {
                yield return new WaitForSeconds(1f);
                Debug.Log($"Wyrzucam liczbê taka sama {randomNumbers}");
                if (decision == 0)
                {
                    spriteRenderer.sprite = spriteArray[randomNumbers];
                }
            ...
            else if (i==3 && randomNumbers != 0)
            {
                randomNumbers -= 1;
                yield return new WaitForSeconds(1f);
                Debug.Log($"Wyrzucam liczbê mniejsz¹ od randoma {randomNumbers}");
                if (decision == 0)
                {
                    spriteRenderer.sprite = spriteArray[randomNumbers];
                }
